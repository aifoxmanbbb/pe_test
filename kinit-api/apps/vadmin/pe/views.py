...
async def _get_self_student_context(db, telephone: str | None, user_id: int | None = None) -> dict[str, Any] | None:
    ...
    if not row:
        logger.warning(
            "[pe.self] student context not found, telephone=%s, user_id=%s",
            telephone, user_id
        )
        return None
...
async def _get_self_score_context(db, telephone: str | None) -> dict[str, Any] | None:
    ...
    if not row:
        logger.warning("[pe.self] score fallback context not found by mobile, telephone=%s", telephone)
        return None
    return {
...
async def get_student_analysis(
    ...
    if not batches:
        logger.info(
            "[pe.analysis.student] empty: no batches, student_no=%s, school=%s, grade=%s, class=%s, stage_type=%s",
    ...
    if not rows:
        logger.info(
            "[pe.analysis.student] empty: no rows, student_no=%s, school=%s, grade=%s, class=%s, batches=%s",
...
async def get_student_analysis_self(
    ...
    logger.info(
        "[pe.self] enter, user_id=%s, telephone=%s, is_staff=%s, query_student_no=%s",
    ...
    if ctx:
        logger.info(
            "[pe.self] student context hit: student_no=%s, school=%s, grade=%s, class=%s",
    ...
    if not isinstance(payload, dict) or not payload.get('profile'):
        ...
        if score_ctx:
            logger.info(
                "[pe.self] score fallback hit: student_no=%s, school=%s, grade=%s, class=%s",
    ...
    if not resp:
        logger.warning("[pe.self] final empty: no context and no fallback")
        return SuccessResponse(_empty_student())
...
async def upsert_scores(
    ...
    for item in scores:
        ...
        if item_code == 'bmi' and (raw_score_raw is None or str(raw_score_raw).strip() == ''):
            ...
            raw_score_parsed = _parse_raw_score(raw_score_raw, item.get('item_code'))
