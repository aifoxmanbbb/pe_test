...
import { getFitnessBatchOptionsApi, upsertFitnessScoresApi, getFitnessBatchListApi } from '@/api/vadmin/fitness'
  ...
  const schema: FormSchema[] = [
    {
      ...
      componentProps: { 
        ...
        onChange: async (val: number) => {
          const batch = (await getFitnessBatchListApi({ id: val })).data.items.find((i: any) => i.id === val)
          if (batch) {
    ...
    <ElCard shadow="never">
      <ElTable :data="studentList" border stripe v-loading="loading">
        ...
        <template v-if="entryMode === 'student'">
          <ElTableColumn v-for="item in itemOptions" :key="item.value" :label="item.label" min-width="100">
