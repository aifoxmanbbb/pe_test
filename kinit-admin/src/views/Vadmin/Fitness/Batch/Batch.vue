...
const loadStandards = async () => {
  const res = await getFitnessStandardListApi().catch(() => null)
  if (res) {
...
const loadList = async () => {
  loading.value = true
  const res = await getFitnessBatchListApi({
    page: page.value,
    limit: limit.value,
    ...searchParams.value
  }).catch(() => null)
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确认删除该批次吗？', '提示', {
    ...
    const res = await deleteFitnessBatchApi(row.id).catch(() => null)
    if (res) {
...
const handleStatus = async (row: any, status: string) => {
  const res = await updateFitnessBatchApi(row.id, { status }).catch(() => null)
  if (res) {
...
const submit = async () => {
  ...
  if (currentId.value) {
    const res = await updateFitnessBatchApi(currentId.value, data).catch(() => null)
    if (res) {
    ...
    const res = await createFitnessBatchApi(data).catch(() => null)
    if (res) {
