...
import { downloadStudentImportTemplateApi, postImportStudentsApi } from '@/api/vadmin/sport'
import { ref } from 'vue'
...
const handleImport = async () => {
  ...
  try {
    const res = await postImportStudentsApi(formData)
    if (res) {
...
const downloadTemplate = async () => {
  ...
  const res = await downloadStudentImportTemplateApi()
  const blob = res?.data
  if (!blob) return
...
<template>
  <div>
    ...
    <ol>
      ...
      <li style="margin-top: 7px">编辑模板文件，将需要导入的学生按模板格式填写完整</li>
      <li style="margin-top: 7px">上传模板文件，点击确认导入</li>
      <li style="margin-top: 7px">查看导入结果，失败数据可下载后修正再导入</li>
    </ol>
  </div>
  <div>
